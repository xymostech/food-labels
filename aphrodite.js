...
/***/ function(module, exports) {
	...
	var BASIS_REGEX = '(?:' + NUMBER_REGEX + '[a-z]+|' + NUMBER_REGEX + '%|auto)';
	var FLEX_REGEX = new RegExp('^\\s*(?:' + ('(' + NUMBER_REGEX + ')|') + ('(' + NUMBER_REGEX + '\\s+' + NUMBER_REGEX + ')|') + ('(' + NUMBER_REGEX + '\\s+' + NUMBER_REGEX + '\\s+' + BASIS_REGEX + ')|') + ('(' + NUMBER_REGEX + '\\s+' + NUMBER_REGEX + '\\s+0)|') + ('(' + NUMBER_REGEX + '\\s+' + BASIS_REGEX + ')|') + ('(' + BASIS_REGEX + ')') + ')\\s*$', "i");
	...
	/**
	 * Parse a CSS "flex" property value.
	 * This takes a string value and parses it into an array of its constituent
	 * 'flex-grow', 'flex-shrink', and 'flex-basis' values. It return null if it
	 * fails to parse the value.
	 *
	 * Example:
	 *  parseFlex("7.2 11em") // -> ["7.2", "1", "11em"]
	 *  parseFlex("not a flex value") // -> null
	 */
	var parseFlex = function parseFlex(string) {
	    var match = string.match(FLEX_REGEX);
	    ...
	    if (string === "none") {
	        ...
	        var _ = _match[0];
	        var justGrow = _match[1];
	        var growAndShrink = _match[2];
	        var growShrinkBasis = _match[3];
	        var growShrinkZeroBasis = _match[4];
	        var growAndBasis = _match[5];
	        var justBasis = _match[6];

	        if (justGrow) {
	            return [justGrow, defaultShrink, defaultBasis];
	        } else if (growAndShrink) {
	            return growAndShrink.split(WHITESPACE_REGEX).concat(defaultBasis);
	        } else if (growShrinkBasis) {
	            return growShrinkBasis.split(WHITESPACE_REGEX);
	        } else if (growShrinkZeroBasis) {
	            var _growShrinkZeroBasis$split = growShrinkZeroBasis.split(WHITESPACE_REGEX);

	            var _growShrinkZeroBasis$split2 = _slicedToArray(_growShrinkZeroBasis$split, 3);

	            var grow = _growShrinkZeroBasis$split2[0];
	            var shrink = _growShrinkZeroBasis$split2[1];
	            var basis = _growShrinkZeroBasis$split2[2];

	            return [grow, shrink, "0%"];
	        } else if (growAndBasis) {
	            var _growAndBasis$split = growAndBasis.split(WHITESPACE_REGEX);

	            var _growAndBasis$split2 = _slicedToArray(_growAndBasis$split, 2);

	            var grow = _growAndBasis$split2[0];
	            var basis = _growAndBasis$split2[1];

	            return [grow, defaultShrink, basis];
	        } else if (justBasis) {
	            return [defaultGrow, defaultShrink, justBasis];
	        }
	    } else {
	        return null;
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var stringHandlers = {
	    ...
	    // IE 10 does a bad job of implementing the `flex` property. In particular:
	    //  - `-ms-flex: 1 !important` doesn't work correctly because of !important
	    //  - `-ms-flex: 1 1 0` isn't valid because it wants a unit on the 0
	    //  - `-ms-flex: 1` gives a default flex-shrink of 0, instead of 1
	    // In order to correct for these problems, we try to parse the flex value
	    // ourselves and always put a flex-grow, flex-shrink, and flex-basis value
	    // to avoid these irregularities (if parsing it fails, we just return the
	    // original value).
	    flex: function flex(val) {
	        var parsed = (0, _util.parseFlex)("" + val);
	        return parsed ? parsed.join(" ") : val;
	    }
	};
